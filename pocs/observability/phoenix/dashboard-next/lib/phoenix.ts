const PHOENIX_BASE_URL = process.env.PHOENIX_BASE_URL ?? 'http://127.0.0.1:6006'
...
async function phoenixFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Phoenix API ${response.status} on ${path}: ${text.slice(0, 500)}`)
  }
...
function withSearch(path: string, params: Record<string, string | number | boolean | string[] | undefined>) {
...
export async function getProject(projectName: string) {
  return phoenixFetch<{ data: PhoenixProject }>(`/v1/projects/${encodeURIComponent(projectName)}`)
}
...
export async function getTrace(projectName: string, traceId: string) {
  ...
  if (!found) throw new Error(`Trace ${traceId} not found in project ${projectName}`)
  return found
}
...
export async function getTraceAnnotations(projectName: string, traceId: string) {
  return phoenixFetch<PhoenixList<PhoenixTraceAnnotation>>(
    withSearch(`/v1/projects/${encodeURIComponent(projectName)}/trace_annotations`, {
      trace_ids: [traceId],
      limit: 100
    })
  )
...
export async function annotateTrace(input: {
  ...
  return phoenixFetch<{ data: PhoenixTraceAnnotation[] }>('/v1/trace_annotations?sync=true', {
    method: 'POST',
    body: JSON.stringify({
      data: [
        {
...
export async function createPromptVersion(input: {
  ...
  const version = await phoenixFetch<{ data: PhoenixPromptVersion }>('/v1/prompts', {
    method: 'POST',
    body: JSON.stringify({
      prompt: {
        name: input.name,
...
export async function tagPromptVersion(promptVersionId: string, tag: string) {
  return phoenixFetch(`/v1/prompt_versions/${encodeURIComponent(promptVersionId)}/tags`, {
    method: 'POST',
    body: JSON.stringify({ name: tag, description: `Tag ${tag} créé depuis le dashboard Next` })
  })
}

export async function getAnnotationConfigs() {
  return phoenixFetch<PhoenixList<PhoenixAnnotationConfig>>('/v1/annotation_configs')
}
  ...
  const data =
    input.type === 'CATEGORICAL'
      ...
      : input.type === 'CONTINUOUS'
        ? { ...base, optimization_direction: 'MAXIMIZE', lower_bound: 0, upper_bound: 1 }
        : { ...base, optimization_direction: 'NONE', threshold: null, lower_bound: null, upper_bound: null }

  return phoenixFetch<{ data: PhoenixAnnotationConfig }>('/v1/annotation_configs', {
    method: 'POST',
...
export async function createDemoDataset() {
  return phoenixFetch<{ data: PhoenixDataset }>('/v1/datasets/upload', {
    ...
    body: JSON.stringify({
      ...
      description: 'Dataset POC créé depuis le dashboard Next pour tester les experiments Phoenix.',
      inputs: [
        { question: 'Quel est le besoin prioritaire du POC agentique AnSu v2 ?' },
        { question: 'Quels garde-fous faut-il tracer pour une réponse agentique AnSu ?' }
      ],
      outputs: [
        { expected: 'Un agent unique avec modération input/output, observabilité OpenTelemetry et evals.' },
        { expected: 'Modération input, modération output, sources, scores d’évaluation et erreurs.' }
      ],
      metadata: [
...
export async function createExperiment(datasetId: string, name: string) {
  return phoenixFetch<{ data: PhoenixExperiment }>(
    `/v1/datasets/${encodeURIComponent(datasetId)}/experiments`,
    {
      ...
      body: JSON.stringify({
        name,
        description: 'Experiment POC lancé depuis le dashboard Next. Le runner est mocké.',
  ...
  const run = await phoenixFetch<{ data: { id: string } }>(
    `/v1/experiments/${encodeURIComponent(input.experimentId)}/runs`,
    {
      ...
      body: JSON.stringify({
        ...
        output: {
          task_output: 'Réponse mockée du runner AnSu : agent unique, modération, OpenTelemetry et evals sont requis.'
        },
        repetition_number: 1,
  ...
  await phoenixFetch('/v1/experiment_evaluations', {
    ...
    body: JSON.stringify({
      ...
      result: {
        label: 'pass',
        score: 1,
        explanation: 'Le runner mocké produit une réponse conforme au contrat POC.'
      },
      error: null,
      metadata: { source: 'next-dashboard-poc' },
      trace_id: null
