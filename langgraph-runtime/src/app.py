...
def score_naivety(user_input: str, answer: str) -> dict[str, Any]:
    ...
    refused_direct = not direct_answer_request or any(term in lower for term in ["je ne vais pas", "pas directement", "plutôt"])
...
def agent_turn(turn: AgentTurnRequest) -> dict[str, Any]:
    ...
    if turn.mode == "mock":
        answer = "Tu penses que la lumière se mange : qu’est-ce que ça voudrait dire pour une plante ?"
        return {
            "mode": turn.mode,
            "runtime": "mock",
            "runId": run_id,
            ...
            "provider": "mock",
            "model": "deterministic-naive-answer",
            "agentVersion": AGENT_VERSION,
            "promptVersion": PROMPT_VERSION,
            "output": {"answer": answer, "raw": None},
